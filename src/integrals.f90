...
subroutine write_integrals_fsi(time,uk,u,work3r,work3c,work1,scalars,Insect,beams,Wings)
  ...
  ! save enstrophy (before multiplying by viscosity)
  enst  = 2.d0*diss   ! note hidden factor of 2 in compute_energies
  enstx = 2.d0*dissx
  ensty = 2.d0*dissy
  enstz = 2.d0*dissz
  enstf  = 2.d0*dissf
  ...
  if(mpirank == 0) then
    ...
    enstf,enstxf,enstyf,enstzf,&
    enst,enstx,ensty,enstz
    close(14)
  endif

  !-----------------------------------------------------------------------------
  ! velocity and vorticity statistics (max and min values)
  !-----------------------------------------------------------------------------
  ! Note: at this point u contains velocity (including solid) in physical space
  ! and work3r contains vorticity (including solid) in physical space
  ! We need to recompute u from uk since it was multiplied by (1-mask) above
  call ifft3 (ink=uk, outx=u)
  
  ! Compute max magnitude for velocity (L2 norm)
  u_max = field_max_magnitude3(u(:,:,:,1:3))
  ! Compute min over all components
  u_min = min(fieldmin(u(:,:,:,1)), fieldmin(u(:,:,:,2)), fieldmin(u(:,:,:,3)))
  
  ! Recompute vorticity (since work3r was multiplied by (1-mask) above)
  call curl( uk, work3c )
  call ifft3( ink=work3c, outx=work3r )
  
  ! Compute max magnitude for vorticity (L2 norm)
  vort_max = field_max_magnitude3(work3r(:,:,:,1:3))
  ! Compute min over all components
  vort_min = min(fieldmin(work3r(:,:,:,1)), fieldmin(work3r(:,:,:,2)), fieldmin(work3r(:,:,:,3)))
  
  if(mpirank == 0) then
    open(14,file='umag.t',status='unknown',position='append')
    write (14,'(3(es15.8,1x))') time, u_max, u_min
    close(14)
    
    open(14,file='vortmag.t',status='unknown',position='append')
    write (14,'(3(es15.8,1x))') time, vort_max, vort_min
    close(14)
  endif
