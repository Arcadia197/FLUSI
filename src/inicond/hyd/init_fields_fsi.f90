...
subroutine init_fields_fsi(time,it,dt0,dt1,n0,n1,uk,nlk,vort,explin,workc,&
    ...
    integer :: i,ix,iy,iz, nxs,nys,nzs, nxb,nyb,nzb,k
    ...
    case ("pressure-blob")
        if (equation /= "artificial-compressibility") call abort(6363,"this inicond is ACM only")
        uk(:,:,:,1:3) = 0.0d0

        do iz=ra(3), rb(3)
            do iy=ra(2), rb(2)
                do ix=ra(1), rb(1)
                    x = dble(ix)*dx - xl / 2.0d0
                    y = dble(iy)*dy - yl / 2.0d0
                    z = dble(iz)*dz - zl / 2.0d0
                    vort(ix,iy,iz,1) = exp( -( (x)**2 + (y)**2 + (z)**2 ) / omega1 )
                enddo
            enddo
        enddo
    ...
    case("ShearLayer")
        ...
        !--------------------------------------------------
        if (mpirank==0) write (*,*) "*** inicond: shear layer initial condition"
        
        ! step 1 & 2 combined: position-based random field generation
        ! create a random vorticity field with proper periodicity and symmetry
        ! using position-based random number generation to avoid MPI communication
        call generate_periodic_symmetric_random_field(vort)
        ...
        k = nint(dsqrt(dble(nx**2 + ny**2 + nz**2)))
        allocate(spec_array(0:k-1,1:2))
        do i=0,k-1
            spec_array(i,1)=i*(2.0d0*pi)/xl

            ! Define target spectrum with a peak at k0 and decay
            k0 = 2.0d0*pi/(7.d0*1)

            ! ! Comte, Lesieur & Lamballais 1992 (CLL)
            ! ! domain size should be 8*7*delta, k0 = 2pi/lambda_a = 2pi/(7*delta)
            ! ! lambda_a is the wavelength of the most energetic structures
            ! ! delta is the shear layer thickness, assumed as 1
            ! spec_array(i,2)= spec_array(i,1)**8 * exp( -4.d0*(spec_array(i,1)/(k0/2.0d0))**2 )

            ! modified by JP to have a less steep low-k behavior
            spec_array(i,2)= (spec_array(i,1)/k0)**4 * exp( -2.d0*(spec_array(i,1)/k0)**2 )
        ...
        do iz=ca(1),cb(1)
            ...
            do iy=ca(2),cb(2)
                ...
                do ix=ca(3),cb(3)
                    ...
                    kabs = dsqrt(kx**2 + ky**2 + kz**2)
                    ! index of spectrum (just the rounded wavenumber, if domain is 2*pi ^3. if not
                    ! the routine will fail.), has to be divided by domain length
                    k = nint(kabs*xl/(2.0d0*pi))
                    ! scale spectrum. (avoid dividing by zero...)
                    if (S_Ekin(k) > 1.0d-13) then
                        uk(iz,iy,ix,1:3) = uk(iz,iy,ix,1:3) * dsqrt(spec_array(k,2) / S_Ekin(k))
                    else
        ...
        ! step 6
        ! check for mean energy and divergence
        deallocate( spec_array )
        ...
        if(root) then
            ...
            write(*,*) "Please note that output of integrals is E=",sum(S_Ekin)*(2.d0*pi)**3
            write(*,*) "because it is the volume integral and not the mean"
            write(*,*) "Maximum divergence in field after spectrum adaptation=", maxdiv
        endif

        ! step 7
        ! modulate with gaussian profile in z-direction in physical space to restrict perturbation to shear layer region
        ! 1.0d0 is the thickness of the shear layer
        ! make sure to add one domain-shifted version as well to incorporate periodic effects to sufficient degree
        call ifft3(uk, vort)
        do iz=ra(3), rb(3)
            z = dble(iz)*dz
            vort(ra(1):rb(1),ra(2):rb(2),iz,1:3) = vort(ra(1):rb(1),ra(2):rb(2),iz,1:3) * &
                (exp( -0.5d0 * ((z - 0.25d0*zl)/1.0d0)**2 ) + exp( -0.5d0 * ((z - 0.75d0*zl)/1.0d0)**2 ) &
                + exp( -0.5d0 * ((z - 0.25d0*zl - zl)/1.0d0)**2 ) + exp( -0.5d0 * ((z - 0.75d0*zl - zl)/1.0d0)**2 ) &
                + exp( -0.5d0 * ((z - 0.25d0*zl + zl)/1.0d0)**2 ) + exp( -0.5d0 * ((z - 0.75d0*zl + zl)/1.0d0)**2 ))
        end do
        ...
        ! step 8
        ! divergence should be non-zero, so we need to project it out
        call divergence( ink=nlk, outk=workc(:,:,:,1) )
        call ifft( workc(:,:,:,1), vort(:,:,:,1) )
        ! vort(:,:,:,1) is now div in phys space
        maxdiv = fieldmax(vort(:,:,:,1))
        if(root) then
            write(*,*) "Maximum divergence in field after gaussian modulation=", maxdiv
        endif

        call curl3_inplace(nlk(:,:,:,1:3,0))
        call Vorticity2Velocity(nlk(:,:,:,1:3,0), uk(:,:,:,1:3))
        ...
        maxdiv = fieldmax(vort(:,:,:,1))
        if(root) then
            write(*,*) "Maximum divergence in field after projection=", maxdiv
        endif
        ...
        ! step 10
        ! add tanh profile in z-direction to u_x to create shear layer, U=1, delta=1
        call ifft3(uk, vort)
        do iz=ra(3), rb(3)
            z = dble(iz)*dz
            vort(ra(1):rb(1),ra(2):rb(2),iz,1) = vort(ra(1):rb(1),ra(2):rb(2),iz,1) &
            - 1.0d0 + 1.0d0 * tanh( 2.0d0*(z - 0.25d0*zl)/1.0d0 ) - 1.0d0 * tanh( 2.0d0*(z - 0.75d0*zl)/1.0d0 )
        end do
        call fft3(vort, uk)
...
!> Generate a periodic and symmetric random vorticity field using position-based random number generation
!! This approach ensures that the same physical position always generates the same random number,
!! naturally providing periodicity and symmetry without MPI communication
subroutine generate_periodic_symmetric_random_field(vort)
    use vars
    implicit none
    
    real(kind=pr), intent(inout) :: vort(ra(1):rb(1),ra(2):rb(2),ra(3):rb(3),1:3)
    integer :: ix, iy, iz, id
    real(kind=pr) :: x, y, z, x_periodic, y_periodic, z_symmetric, z_mirror
    real(kind=pr) :: rnd_val
    integer :: seed_offset

    ! vary this to give different random fields
    seed_offset = 0
    
    do id = 1, 3  ! for each vorticity component
        seed_offset = seed_offset + 1000 * id  ! different seed offset for each component
        ...
        do iz = ra(3), rb(3)
            ...
            do iy = ra(2), rb(2)
                ...
                do ix = ra(1), rb(1)
                    ...
                    ! get random number based on periodic/symmetric position
                    call random_number_from_position(x, y, z, seed_offset, rnd_val)
                    vort(ix,iy,iz,id) = rnd_val
                    
                    ! apply symmetry constraints for shear layer - second half: symmetric for components 1&2, antisymmetric for component 3
                    if (iz >= nz/2 .and. id /= 3) vort(ix,iy,iz,id) = -vort(ix,iy,iz,id)
                    
                    ! set symmetry planes to zero
                    if (iz == 0 .or. iz == nz/2) vort(ix,iy,iz,id) = 0.0d0
                enddo
            enddo
        enddo
    enddo
    
end subroutine generate_periodic_symmetric_random_field


!> Generate a random number for a given position using hash-based seeding
!! This ensures that the same position always gives the same random number
subroutine random_number_from_position(x, y, z, seed_offset, rnd_val)
    use vars
    implicit none
    
    real(kind=pr), intent(in) :: x, y, z           ! position coordinates
    integer, intent(in) :: seed_offset             ! seed offset for variation
    real(kind=pr), intent(out) :: rnd_val          ! output random value
    
    integer :: nseed, ir
    real(kind=pr) :: dx_min(1:3)
    integer, allocatable :: seed(:)
    integer(kind=8) :: hashval
    
    ! compute position tolerance based on grid spacing
    dx_min(1) = 1.0e-3_pr * dx
    dx_min(2) = 1.0e-3_pr * dy
    dx_min(3) = 1.0e-3_pr * dz

    call random_seed(size=nseed)
    allocate(seed(nseed))
    
    ! compute a unique hash for the position and seed offset
    ...
    hashval = ieor(hashval, int(seed_offset,kind=8) * 69069_8)
    
    ! fill the seed array with the hash value
    seed = 0
    seed(1) = int(ishft(hashval, -32))
    if (nseed > 1) seed(2) = int(iand(hashval, Z'FFFFFFFF'))
    
    ! set the random seed based on position and seed offset
    call random_seed(put=seed)
    
    ! warm up the random number generator and get final value
    do ir = 1, 10
        rnd_val = rand_nbr() - 0.5d0  ! range [-0.5, 0.5]
    enddo
    
    deallocate(seed)
end subroutine random_number_from_position
