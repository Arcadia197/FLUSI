module module_helpers
    use vars, only: strlen, pr, pi, abort
    use mpi
    implicit none
...
contains
    ...
    character(len=strlen)  function get_dsetname(fname)
        ...
        ! if no "_" is found, then we are printing an error
        if (index(fname, '_', .true.) == 0) then
            call abort(001, "Filename "//trim(adjustl(fname))//" does not follow FLUSI conventions ([NAME]_[TIME].h5).")
        endif
