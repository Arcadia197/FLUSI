...
subroutine cal_nlk(time,it,nlk,uk,u,vort,work,workc,press,scalars,scalars_rhs,Insect,beams,wings)
  ...
  case("fsi")
      ...
      case ("navier-stokes", "navier-stokes-SS")
          ! compute source-terms, *not* divergence-free
          t1 = MPI_wtime()
          if (equation == "navier-stokes") then
            call cal_nlk_fsi(time,it,nlk,uk,u,vort,work,workc,Insect)
          else
            call cal_nlk_fsi_skew_symmetric(time,it,nlk,uk,u,vort,work,workc,Insect)
          endif
...
!          in this routine, which is dead weight when not using this function.
!-------------------------------------------------------------------------------
subroutine cal_nlk_fsi_skew_symmetric(time,it,nlk,uk,u,vort,work,workc, Insect)
  use mpi
  use p3dfft_wrapper
  use vars
  use module_insects
  ...
  type(diptera),intent(inout) :: Insect
  real(kind=pr) :: t0,t1,ux,uy,uz,vorx,vory,vorz,chi,usx,usy,usz
  real(kind=pr) :: fx,fy,fz,fx1,fy1,fz1, kx, ky, kz
  real(kind=pr) :: soft_startup, dt, eps_inv
  integer :: ix,iz,iy,mpicode, i,j
  t0 = MPI_wtime()
  fx=0.d0; fy=0.d0; fz=0.d0
  ...
  !-----------------------------------------------------------------------------
  t1 = MPI_wtime()
  call curl ( ink=uk, outk=workc )
  ! transform it to physical space
  call ifft3 ( ink=workc, outx=vort )
  call toc("RHS (cal_nlk_fsi::curl+ifft3)", MPI_wtime() - t1)
  ...
  !-----------------------------------------------------------------------------
  !-- Non-Linear terms (skew-symmetric form)
  !
  !   In order to use divergence form, comment step 2 and use different
  !   expression for nlk in step 3
  !   
  !   In order to use energy form, comment step 3
  !-----------------------------------------------------------------------------
  t1 = MPI_wtime()
  ! Step 1: u is already in physical space
  ! Step 2: Compute divergence of u_i u_j for each component
  do i = 1, 3
      ! Compute u_i * u_j in physical space and transform to spectral
      do j = 1, 3
          work(:,:,:,j) = u(:,:,:,i) * u(:,:,:,j)
          call fft( inx=work(:,:,:,j), outk=workc(:,:,:,j) )
      enddo
      ...
      do ix=ca(3),cb(3)
          ...
          do iy=ca(2),cb(2)
              ...
              do iz=ca(1),cb(1)
                  kz=wave_z(iz)
                  nlk(iz,iy,ix,i) = -dcmplx(0.d0,1.d0)*kx*workc(iz,iy,ix,1) &
                                  -  dcmplx(0.d0,1.d0)*ky*workc(iz,iy,ix,2) &
                                  -  dcmplx(0.d0,1.d0)*kz*workc(iz,iy,ix,3)
              enddo
          enddo
      enddo
  enddo

  ! Step 3: Compute u_j * grad_j u_i for each component
  do i = 1, 3
      ! Compute grad_j u_i in spectral space
      do ix=ca(3),cb(3)
          kx=wave_x(ix)
          do iy=ca(2),cb(2)
              ky=wave_y(iy)
              do iz=ca(1),cb(1)
                  kz=wave_z(iz)
                  workc(iz,iy,ix,1) = dcmplx(0.d0,1.d0)*kx*uk(iz,iy,ix,i)
                  workc(iz,iy,ix,2) = dcmplx(0.d0,1.d0)*ky*uk(iz,iy,ix,i)
                  workc(iz,iy,ix,3) = dcmplx(0.d0,1.d0)*kz*uk(iz,iy,ix,i)
              enddo
          enddo
      enddo
      ! Transform to physical space
      do j = 1, 3
        call ifft( ink=workc(:,:,:,j), outx=work(:,:,:,j) )
      enddo

      ! Compute u_j * grad_j u_i in physical space and sum
      work(:,:,:,1) = sum(u(:,:,:,1:3) * work(:,:,:,1:3), dim=4)
      ! Transform to spectral and add to nlk
      call fft( inx=work(:,:,:,1), outk=workc(:,:,:,1) )
      do ix=ca(3),cb(3)
          do iy=ca(2),cb(2)
              do iz=ca(1),cb(1)
                  nlk(iz,iy,ix,i) = 0.5_pr * ( nlk(iz,iy,ix,i) - workc(iz,iy,ix,1) )
                  ! nlk(iz,iy,ix,i) = -workc(iz,iy,ix,1)  ! only u grad u terms are used
              enddo
          enddo
      enddo
  enddo

  ! Penalization term (as before)
  do iz=ra(3),rb(3)
      do iy=ra(2),rb(2)
          do ix=ra(1),rb(1)
              chi = mask(ix,iy,iz) * eps_inv
              ...
              usy = us(ix,iy,iz,2)
              usz = us(ix,iy,iz,3)
              fx = fx + chi*(u(ix,iy,iz,1)-usx)
              fy = fy + chi*(u(ix,iy,iz,2)-usy)
              fz = fz + chi*(u(ix,iy,iz,3)-usz)
              ! Add penalization in physical space, then transform and add to nlk
              work(ix,iy,iz,1) = -chi*(u(ix,iy,iz,1)-usx)
              work(ix,iy,iz,2) = -chi*(u(ix,iy,iz,2)-usy)
              work(ix,iy,iz,3) = -chi*(u(ix,iy,iz,3)-usz)
          enddo
      enddo
  enddo
  call fft3( inx=work, outk=workc )
  nlk(:,:,:,1:3) = nlk(:,:,:,1:3) + workc(:,:,:,1:3)
  call toc("RHS (cal_nlk_fsi_skew_symmetric::skew-symmetric NL)", MPI_wtime() - t1)
  ...
  ! save timing
  call toc( "RHS (cal_nlk_fsi)", MPI_wtime() - t0)
end subroutine cal_nlk_fsi_skew_symmetric
